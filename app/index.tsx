...
// Import Clarifai food mapping for ingredient detection
const CLARIFAI_FOOD_MAPPING: { [key: string]: { name: string, carbsPer100g: number, weight: number, category: string, ignore?: boolean, note?: string } } = require('../assets/clarifai-food-mapping.json');
  ...
  useEffect(() => {
    const initClarifai = async () => {
      try {
        ...
        console.log('Clarifai food mapping with', Object.keys(CLARIFAI_FOOD_MAPPING).length, 'food categories available');
        
        setIsClarifaiReady(true);
      } catch (error) {
        console.error('Error initializing Clarifai:', error);
        setIsClarifaiReady(true); // Still allow app to work in demo mode
      }
    };
...
export function LoadingScreen() {
  return (
    <View style={{ flex: 1, justifyContent: "center", alignItems: "center" }}>
      <Text style={{ fontSize: 20 }}>Loading Clarifai Food Recognition...</Text>
      <Text style={{ fontSize: 14, marginTop: 10, textAlign: 'center', color: '#666' }}>
        Preparing to identify food items for carb estimation
      </Text>
    </View>
  );
}
...
export function CameraScreen() {
  ...
  const renderAllDetections = () => {
    ...
    // Separate detections for better display
  ...
  const takePicture = async () => {
    try {
      ...
      const photo = await ref.current.takePictureAsync({
        quality: 0.8, // Increased from 0.5 to 0.8 for better quality
        base64: false,
          ...
          <View style={{ flex: 1 }}>
            ...
            <View style={{ flexDirection: 'row', padding: 16 }}>
              <Button 
                ...
                onPress={() => setShowAllDetections(false)}
              ...
              <Button 
                mode={showAllDetections ? "contained" : "outlined"}
                onPress={() => setShowAllDetections(true)}
                style={{ flex: 1 }}
              >

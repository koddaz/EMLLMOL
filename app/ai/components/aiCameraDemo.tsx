  ...
  return (
    <ScrollView style={{ flex: 1, padding: 16 }}>
      ...
      <Card style={{ marginBottom: 16, padding: 16 }}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
          ...
          <Chip 
            mode="outlined"
            style={{ 
              backgroundColor: initStatus === 'ready' 
          ...
          >
            {initStatus === 'ready' ? '✅ Ready' : 
             ...
             initStatus === 'initializing' ? '⏳ Loading' : '🔄 Idle'}
          </Chip>
        </View>

        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
          <Text variant="bodyMedium">Inference: </Text>
          <Chip 
            mode="outlined"
            style={{ 
              backgroundColor: isUsingRealInference() 
                ? theme.colors.primaryContainer 
                : theme.colors.secondaryContainer
            }}
          >
            {isUsingRealInference() ? '🚀 ONNX Runtime' : '🧪 Mock'}
          </Chip>
        </View>

        <Text variant="bodySmall" style={{ color: theme.colors.outline, marginBottom: 12 }}>
          {initStatus === 'ready' 
            ? `ONNX Runtime service ready with ${modelInfo.numClasses} classes (${isUsingRealInference() ? 'real' : 'mock'} inference)`
            : initStatus === 'error'
            ? 'ONNX Runtime service failed to initialize. Check logs for details.'
            : initStatus === 'initializing'
            ? 'Initializing ONNX Runtime model and services...'
            : 'ONNX Runtime service not initialized'}
        </Text>
        ...
        >
          {initStatus === 'error' ? 'Retry' : 'Refresh'} Status
        </Button>
      </Card>
      ...
      <Card style={{ marginBottom: 16, padding: 16 }}>
        ...
        <View style={{ gap: 8 }}>
          <Button 
            ...
            disabled={isAnalyzing || initStatus !== 'ready'}
      ...
      {isAnalyzing && (
        <Card style={{ marginBottom: 16, padding: 16 }}>
          <Text variant="titleMedium" style={{ marginBottom: 12 }}>
            🔄 ONNX Runtime Analysis Running...
          </Text>
          <ProgressBar indeterminate style={{ marginBottom: 8 }} />
          <Text variant="bodySmall" style={{ color: theme.colors.outline }}>
            {isUsingRealInference() 
              ? 'Running ONNX Runtime inference on device'
              : 'Running mock inference for development'
            }
          </Text>
          <Text variant="bodySmall" style={{ color: theme.colors.outline, marginTop: 4 }}>
            Classifying carb foods and estimating portions
          </Text>
        </Card>
      )}
      ...
      <Card style={{ marginBottom: 16, padding: 16 }}>
        ...
        <List.Item
          title="Runtime"
          description={`ONNX Runtime ${isUsingRealInference() ? '(Native)' : '(Mock for Development)'}`}
          left={props => <List.Icon {...props} icon="lightning-bolt" />}
        />
        <List.Item
          title="Model Type"
          description="ResNet-18 Carb Classifier (.onnx format)"
          left={props => <List.Icon {...props} icon="brain" />}
        />
        <List.Item
          title="Supported Foods"
          description={`${modelInfo.numClasses} carb categories: ${modelInfo.classNames.slice(0, 3).join(', ')}...`}
          left={props => <List.Icon {...props} icon="food-variant" />}
        />
        <List.Item
          title="Input Size"
          description="224x224 RGB pixels"
          left={props => <List.Icon {...props} icon="image-size-select-actual" />}
        />
        <List.Item
          title="Model Accuracy"
          description={`${modelInfo.accuracy.toFixed(1)}% classification accuracy`}

...
export class ONNXService {
  ...
  private async loadModelConfig(): Promise<void> {
    try {
      const configModule = require('../models/json/mobile_config.json');
  ...
  private async loadONNXModel(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        const modelAsset = Asset.fromModule(require('../models/carb_classifier.onnx'));
  ...
  async processImage(imageUri: string): Promise<{
    ...
    try {
      ...
      // Enhanced thresholding for non-food detection
      const confidenceThreshold = 0.70; // 70% minimum confidence - much higher!
      const competitionThreshold = 0.30; // Second best must be <30% for clear classification
      const secondBest = allPredictions[1];
      
      const isConfidentClassification = bestPrediction.confidence >= confidenceThreshold;
      const isClearWinner = (bestPrediction.confidence - secondBest.confidence) > competitionThreshold;

      if (isConfidentClassification && isClearWinner) {
        ...
        const reason = !isConfidentClassification 
          ? `Low confidence: ${(bestPrediction.confidence * 100).toFixed(1)}% < 70%`
          : `Too close: ${bestPrediction.className} ${(bestPrediction.confidence * 100).toFixed(1)}% vs ${secondBest.className} ${(secondBest.confidence * 100).toFixed(1)}%`;
          
        console.log(`❌ No clear food classification - ${reason}`);
        console.log('💡 This might not be food, or try pointing camera at: beans, bread, corn, grains, pancakes, pasta, pizza, potato, or rice');
        console.log('📊 Top predictions:', allPredictions.slice(0, 3).map(p => `${p.className}: ${(p.confidence * 100).toFixed(1)}%`).join(', '));
